...
def RankAndCrowdingSurvival(networks, k):
    result = []
    fitness = [x[0] for x in networks]
    pareto_fronts = sortNondominated(fitness, k)
    crowding = CrowdingDist(fitness)
    chosen = list(chain(*pareto_fronts[:-1]))

    k = k - len(chosen)
    if k > 0:
        sorted_front = [x for _,x in sorted(zip(crowding ,pareto_fronts[-1]), reverse=True)]
        chosen.extend(sorted_front[:])

    networks_list = [x[1] for x in networks]
    for data in chosen:
        networks_list[data].front = True
        result.append(networks_list[data])
        
    return result

def get_sorted_RankAndCrowdingSurvival(networks, k):
    result = []
    fitness = [x[0] for x in networks]
    pareto_fronts = sortNondominated(fitness, k)
    crowding = CrowdingDist(fitness)
...
# input = [(91,0,20), (1,4,0), (5,1,0), (5,5,5), (6,0,20), (0,0,0), (1,0,0)]
# print(type(input))
# print(RankAndCrowdingSurvival(input,7))