...
# Xiaochu Style
# (R,G,B)
color1 = [(179, 0, 0), (228, 26, 28), (255, 255, 51), (49, 163, 84), (0, 109, 45), (255, 255, 51),
          (240, 2, 127), (240, 2, 127), (240, 2, 127), (240, 2, 127), (240, 2, 127),
          (217, 95, 14), (254, 153, 41), (255, 255, 51), (44, 127, 184), (0, 0, 255)
          ]
...
def parse_args():
    ...
    # csonas, darts, shufflenetv2, mobilenetv2, mnasnet, nasnet
    parser.add_argument('--model', help='which model is used as backbone', type=str, default='nsgac2')
    parser.add_argument('--image_path', help='Path of COCO val images', type=str, default='dataset/val2017/')
    parser.add_argument('--gt_anno', help='Path of COCO val annotation', type=str, default="dataset/annotations/person_keypoints_val2017.json")
    parser.add_argument('--save_path', help="Path to save the visualizations", type=str, default='visualization/coco/')
    parser.add_argument('--style', help="Style of the visualization: Chunhua style or Xiaochu style", type=str, default='chunhua')
    args = parser.parse_args()
    args.prediction = "output/coco/pose_{}/coco_256x192_d256x3_adam_lr1e-3/results/keypoints_val2017_results.json".format(args.model)
    args.save_path = 'visualization/coco/pose_{}/'.format(args.model)
...
def plot(data, gt_file, img_path, save_path, link_pairs, ring_color, save=True):
    # joints
    coco = COCO(gt_file)
    ...
    for catId in catIds:
        for imgId in p.imgIds[:5000]:
                    ...
                    if iou < 0.1 or score < threshold:
                        ...
                        for k, link_pair in enumerate(link_pairs):
                            if link_pair[0] in joints_dict and link_pair[1] in joints_dict:
                                if dt_joints[link_pair[0], 2] < joint_thres or dt_joints[link_pair[1], 2] < joint_thres \
                                        or vg[link_pair[0]] == 0 or vg[link_pair[1]] == 0:
                                    continue
                            if k in range(6, 11):
                                lw = 1
                            else:
                        ...
                        for k in range(dt_joints.shape[0]):
                            if dt_joints[k, 2] < joint_thres or vg[link_pair[0]] == 0 or vg[link_pair[1]] == 0:
                                continue
            ...
            plt.margins(0, 0)
            if save:
                # plt.savefig(save_path + 'score_' + str(np.int(avg_score)) + '_id_' + str(imgId) + '_' + img_name + '.png',
                #             format='png', bbox_inckes='tight', dpi=100)
                plt.savefig(args.save_path + 'id_' + str(imgId) + '_' + img_name + '.png', format='png', bbox_inckes='tight', dpi=100)
                # plt.savefig(save_path + 'id_' + str(imgId) + '.pdf', format='pdf', bbox_inckes='tight', dpi=100)
            # plt.show()
            plt.close()
