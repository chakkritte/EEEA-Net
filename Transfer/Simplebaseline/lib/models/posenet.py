...
from .backbones import pairnas, darts, nasnet, mnasnet, mobilenetv2, shufflenetv2, resnet18, moea, moeasota, moea16, moeasota12, nsgac1, nsgac2, mobilenetv3
...
class PoseNet(nn.Module):
    def __init__(self, cfg, is_train=True):
        ...
        if is_train:
            if model_name.startswith('nsga'):
                self.backbone.load_state_dict(torch.load(cfg.MODEL.PRETRAINED+'/net.inherited'), strict=True)
            else:
                self.backbone.load_state_dict(torch.load(cfg.MODEL.PRETRAINED), strict=True)
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained) or 'net-flops' in pretrained:
