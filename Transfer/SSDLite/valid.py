...
if __name__ == '__main__':
    ...
    checkpoint_dir = 'outputs/voc-320-{}-cosine-e200-lr0.010'.format(args.net)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        result = torch.cat([indexes.reshape(-1, 1), labels.reshape(-1, 1).float(), probs.reshape(-1, 1), boxes], dim=1)
        if result.size(1) == 7:
