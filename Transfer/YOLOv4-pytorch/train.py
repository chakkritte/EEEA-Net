...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id, accumulate, fp_16):
        ...
        self.val_dataloader = DataLoader(self.train_dataset,
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                label_sbbox = label_sbbox.cuda(non_blocking=True)
                label_mbbox = label_mbbox.cuda(non_blocking=True)
                label_lbbox = label_lbbox.cuda(non_blocking=True)
                sbboxes = sbboxes.cuda(non_blocking=True)
                mbboxes = mbboxes.cuda(non_blocking=True)
                lbboxes = lbboxes.cuda(non_blocking=True)
            ...
            self.__save_model_weights(epoch, 0.1)
            
            # self.yolov4.eval()

            # for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.val_dataloader):
            ...
            #         logger.info("  === Val Epoch:[{:3}/{}],step:[{:3}/{}],total_loss:{:.4f}|loss_ciou:{:.4f}|loss_conf:{:.4f}|loss_cls:{:.4f}".format(
            #             epoch, self.epochs,i, len(self.train_dataloader) - 1,mloss[3], mloss[0], mloss[1],mloss[2]
            #         ))
            #         writer.add_scalar('loss_ciou', mloss[0],
            #                           len(self.train_dataloader) / (cfg.TRAIN["BATCH_SIZE"]) * epoch + i)
            #         writer.add_scalar('loss_conf', mloss[1],
            ...
            if epoch >= 0 and cfg.TRAIN["DATA_TYPE"] == 'COCO':
