...
class NSGA(nn.Module):
    def __init__(self, weight_path=None,  resume=False, extract_list=["6", "13", "conv"], feature_channels=[32, 96, 1280]):
        super(NSGA, self).__init__()
        
        config = json.load(open(weight_path+'/net.config'))
        subnet = json.load(open(weight_path+'/net.subnet'))
        self.__submodule = NSGANetV2.build_from_config(config, depth=subnet['d'])

        if weight_path and not resume:
            print("*"*40, "\nLoading weight of NSGA : {}".format(weight_path))
            init = torch.load(weight_path+'/net.inherited', map_location='cpu')
            self.__submodule.load_state_dict(init)
            print("Loaded weight of NSGA : {}".format(weight_path))

        from torch.autograd import Variable
        input = Variable(torch.randn(3, 3, 224, 224))
        out = self.__submodule(input)
        self.feature_channels = self.__submodule.channels

    def forward(self, x):
        return self.__submodule(x)

def _BuildNSGA(weight_path,resume):
    model = NSGA(weight_path=weight_path, resume=resume)
    return model, model.feature_channels