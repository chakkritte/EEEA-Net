...
from inplace_abn import InPlaceABN as BatchNorm2d
from backbones import pairnas, darts, nasnet, mnasnet, mobilenetv2, shufflenetv2, resnet18, moea, moea_sota, moea16, moea_sota12, nsgac1, nsgac2, mobilenetv3
...
pretrained_paths = {
    ...
    'nsgac1': 'net-flops@137-n',
    'nsgac2': 'net-flops@217-n',
...
class ContextPath(nn.Module):
    def __init__(self, net_name=None, is_train=True):
        ...
        if net_name.startswith('nsga'):
            self.net = eval(net_name)('pretrained_models/{}'.format(pretrained_paths[net_name]))
        ...
        if is_train:
            ...
            if net_name.startswith('nsga'):
                self.net.load_state_dict(torch.load('pretrained_models/{}'.format(pretrained_paths[net_name])+'/net.inherited'), strict=False)
            else:
                self.net.load_state_dict(torch.load('pretrained_models/{}'.format(pretrained_paths[net_name])), strict=False)
