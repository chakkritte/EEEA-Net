...
class CityScapes(Dataset):
    def __init__(self, rootpth, cropsize_wh=(640, 480), mode='train', *args, **kwargs):
        ...
        with open(osp.join(rootpth, mode+'.txt'), 'r') as fr:
            pairs = fr.read().splitlines()
        self.img_paths, self.lb_paths = [], []
        for pair in pairs:
            imgpth, lbpth = pair.split(',')
            self.img_paths.append(osp.join(rootpth, imgpth))
            self.lb_paths.append(osp.join(rootpth, lbpth))

        assert len(self.img_paths) == len(self.lb_paths)
        self.len = len(self.img_paths)

        self.imgs = {}
        self.imnames = [ osp.basename(el.replace('_leftImg8bit.png', '')) for el in self.img_paths]
        self.imgs.update(dict(zip(self.imnames, self.img_paths)))

        self.labels = {}
        names = [ osp.basename(el.replace('_gtFine_labelIds.png', '')) for el in self.lb_paths]
        self.labels.update(dict(zip(names, self.lb_paths)))
        ...
        # impths = [os.listdir(osp.join(impth, el)) for el in im_names]
        ...
        # lbpths = [os.listdir(osp.join(gtpth, el)) for el in lbnames]
