...
class Network():
    ...
    def train(self, dataset, generation, db, api):
        ...
        """
        #if self.accuracy == 0.:
        arch = nasnet_setup.decode(self.network)
        index = api.query_index_by_arch(arch.tostr())
        results = api.query_by_index(index, 'cifar100') 

        for seed, result in results.items():
            self.params = result.params
            self.flops = result.flop
            self.accuracy = result.get_eval('x-test')['accuracy']
#         self.accuracy , self.params, self.flops, self.epoch_time = train_and_score(self.network, dataset, generation, self.hash)
    ...
    def load_infomation(self, accuracy, params, flops):
