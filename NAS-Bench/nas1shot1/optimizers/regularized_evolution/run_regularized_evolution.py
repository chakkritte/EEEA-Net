...
def train_and_eval(config):
    ...
    return nasbench_data['validation_accuracy'], nasbench_data['test_accuracy'], nasbench_data['training_time'], nasbench_data['trainable_parameters']
...
def regularized_evolution(cycles, population_size, sample_size, early_exit, ee_params):
    ...
    while len(population) < population_size:
        if early_exit == True:
            ...
            model.validation_accuracy, model.test_accuracy, model.training_time, params = train_and_eval(model.arch)
            if params <= ee_params:
    ...
    while len(history) < cycles:
        ...
        child.validation_accuracy, child.test_accuracy, child.training_time, params = train_and_eval(child.arch)
...
with open(os.path.join(args.output_path, "discrete_optimizers", 'config.json'), 'w') as fp:
        ...
        for seed in range(args.n_repetitions):
            ...
            if alg == 'RE':
                history = regularized_evolution(
                    cycles=args.n_iters, population_size=args.pop_size, sample_size=args.sample_size, early_exit=False, ee_params=0)
            elif alg == 'EE':
                history = regularized_evolution(
                    cycles=args.n_iters, population_size=args.pop_size, sample_size=args.sample_size, early_exit=True, ee_params=3.5e7)
            else:
